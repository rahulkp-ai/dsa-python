...
from src.dynamic_programming.dp_algorithms import coin_change, lis
...
from src.sorting.sorting_algorithms import heap_sort, merge_sort, quick_sort
...
def test_dijkstra_dense():
    n = 200
    g = {
        i: [(j, abs(i - j)) for j in range(max(0, i - 5), min(n, i + 5)) if j != i]
        for i in range(n)
    }
    d = dijkstra(g, 0)
