...
def test_rotated_none():
    assert search_rotated([4, 5, 6, 7, 0, 1, 2], 3) == -1


def test_peak():
    assert [1, 2, 3, 1][find_peak([1, 2, 3, 1])] == 3


def test_sqrt():
    assert isqrt(4) == 2 and isqrt(8) == 2 and isqrt(9) == 3
...
def test_matrix_none():
    assert not search_matrix([[1, 3, 5, 7], [10, 11, 16, 20]], 13)
...
def test_ship():
    assert ship_days(list(range(1, 11)), 5) == 15
