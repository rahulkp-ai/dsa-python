...
# --- BFS & DFS Traversal ---
def test_bfs_starts(g):
    assert bfs(g, 0)[0] == 0


def test_bfs_all(g):
    assert set(bfs(g, 0)) == {0, 1, 2, 3, 4, 5}


def test_dfs_starts(g):
    assert dfs(g, 0)[0] == 0


def test_dfs_all(g):
    assert set(dfs(g, 0)) == {0, 1, 2, 3, 4, 5}
...
def test_bfs_path_none(g):
    assert bfs_shortest_path(g, 3, 5) is None
...
def test_course_schedule():
    assert course_schedule(4, [[1, 0], [2, 0], [3, 1]])


def test_course_cycle():
...
def test_islands():
    assert num_islands([["1", "1", "0"], ["1", "1", "0"], ["0", "0", "1"]]) == 2
