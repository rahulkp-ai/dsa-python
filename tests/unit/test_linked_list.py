...
from src.linked_list.linked_list import (
    SinglyLinkedList, ListNode, DoublyLinkedList, DListNode,
    merge_sorted_lists, remove_nth_from_end, is_palindrome
)

def make_ll(*vals): return SinglyLinkedList.from_list(list(vals))
...
def test_insert_head():
    ll = SinglyLinkedList()
    for v in [3, 2, 1]: ll.insert_head(v)
    assert ll.to_list() == [1, 2, 3]
...
def test_palindrome_true(): assert is_palindrome(make_ll(1, 2, 2, 1).head)
def test_palindrome_false(): assert not is_palindrome(make_ll(1, 2, 3).head)
def test_palindrome_single(): assert is_palindrome(make_ll(1).head)
def test_palindrome_empty(): assert is_palindrome(None)
