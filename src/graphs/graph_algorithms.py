...
def bfs_shortest_path(graph: Dict, start: int, end: int) -> Optional[List[int]]:
    ...
    visited = {start}; queue = deque([(start, [start])])
    while queue:
        ...
        for nb in graph.get(node, []):
            if nb not in visited:
                ...
                visited.add(nb); queue.append((nb, new_path))
    return None
...
def topological_sort(n: int, edges: List[List[int]]) -> Optional[List[int]]:
    ...
    indeg = [0] * n; adj: Dict = defaultdict(list)
...
class UnionFind:
    ...
    def __init__(self, n: int) -> None:
        self.p = list(range(n)); self.rank = [0]*n; self.components = n
...
def kruskal_mst(n: int, edges: List[Tuple]) -> Tuple[int, List]:
    ...
    uf = UnionFind(n); mst: List = []; total = 0
    for w, u, v in sorted(edges):
        if uf.union(u, v): mst.append((w, u, v)); total += w
