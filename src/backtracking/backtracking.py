...
def solve_n_queens(n: int) -> List[List[str]]:
    ...
    """
    res: List[List[str]] = []
    cols: set = set(); d1: set = set(); d2: set = set(); queens: List[int] = []
    def bt(row: int) -> None:
        if row == n:
        ...
        for col in range(n):
            ...
            bt(row + 1)
            queens.pop(); cols.remove(col); d1.remove(row-col); d2.remove(row+col)
    bt(0); return res
...
def permutations(nums: List[int]) -> List[List[int]]:
    ...
    def bt(cur: List[int], rem: List[int]) -> None:
        ...
        for i in range(len(rem)):
            cur.append(rem[i]); bt(cur, rem[:i]+rem[i+1:]); cur.pop()
    bt([], nums); return res
...
def combination_sum(candidates: List[int], target: int) -> List[List[int]]:
    ...
    [[2, 2, 3], [7]]
    """
    candidates.sort(); res: List[List[int]] = []
...
def letter_combinations(digits: str) -> List[str]:
    ...
    def bt(i: int, cur: List[str]) -> None:
        if i == len(digits): res.append("".join(cur)); return
        for ch in m[digits[i]]: cur.append(ch); bt(i+1, cur); cur.pop()
    bt(0, []); return res
