...
def factorial_mod(n: int, mod: int) -> int:
    ...
    """
    result = 1
    for i in range(2, n+1): result = result*i % mod
    return result
...
def nCr_mod(n: int, r: int, mod: int) -> int:
    ...
    if r > n: return 0
    num = den = 1
    for i in range(r): num = num*(n-i)%mod; den = den*(i+1)%mod
...
def nth_fibonacci_matrix(n: int) -> int:
    ...
    def mat_pow(M: List[List[int]], p: int) -> List[List[int]]:
        if p == 1: return M
        if p % 2 == 0: half = mat_pow(M, p//2); return mat_mul(half, half)
        return mat_mul(M, mat_pow(M, p-1))
    if n <= 1: return n
    M = [[1,1],[1,0]]; result = mat_pow(M, n)
    return result[0][1]
