...
def tower_of_hanoi(n: int, src: str="A", dst: str="C", aux: str="B") -> List[str]:
    ...
    7
    """
    if n == 1: return [f"Move disk 1: {src} -> {dst}"]
    moves: List[str] = []
...
def merge_sort_recursive(arr: List[int]) -> List[int]:
    ...
    mid = len(arr)//2
    l = merge_sort_recursive(arr[:mid]); r = merge_sort_recursive(arr[mid:])
    res: List[int] = []; i = j = 0
...
def binary_search_rec(nums: List[int], target: int, l: int=0, r: int=-1) -> int:
    ...
    elif nums[mid] < target: return binary_search_rec(nums, target, mid+1, r)
    else: return binary_search_rec(nums, target, l, mid-1)
