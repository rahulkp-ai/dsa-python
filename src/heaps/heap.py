...
class MinHeap:
    ...
    def pop(self) -> int:
        ...
        while True:
            s = i
            l, r = 2 * i + 1, 2 * i + 2
            if l < n and self._d[l] < self._d[s]:
                s = l
            if r < n and self._d[r] < self._d[s]:
                s = r
            if s == i:
                break
    ...
    def heapify(cls, arr: List[int]) -> "MinHeap":
        ...
        for i in range(len(h._d) // 2 - 1, -1, -1):
            ...
            while True:
                s = j
                l, r = 2 * j + 1, 2 * j + 2
                if l < n and h._d[l] < h._d[s]:
                    s = l
                if r < n and h._d[r] < h._d[s]:
                    s = r
                if s == j:
                    break
                h._d[j], h._d[s] = h._d[s], h._d[j]
                j = s
        return h
...
if __name__ == "__main__":
    ...
    for x in [1, 2, 3, 4, 5]:
        mf.add_num(x)
        print(f"  After {x}: median={mf.find_median()}")
