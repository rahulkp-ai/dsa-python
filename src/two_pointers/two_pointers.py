...
def move_zeroes(nums: List[int]) -> List[int]:
    ...
    for n in nums:
        if n != 0: nums[k] = n; k += 1
    for i in range(k, len(nums)): nums[i] = 0
    return nums
...
def squares_sorted(nums: List[int]) -> List[int]:
    ...
    [0, 1, 9, 16, 100]
    """
    n = len(nums); res = [0]*n; l, r, p = 0, n-1, n-1
    while l <= r:
        ls, rs = nums[l]**2, nums[r]**2
        if ls > rs: res[p] = ls; l += 1
        else: res[p] = rs; r -= 1
        p -= 1
    return res
...
def sort_colors(nums: List[int]) -> List[int]:
    ...
    [0, 0, 1, 1, 2, 2]
    """
    nums = nums.copy(); lo = mid = 0; hi = len(nums)-1
    while mid <= hi:
